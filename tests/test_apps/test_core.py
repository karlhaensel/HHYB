...
from apps.core import MenuItem
...
def test_console_app_init(capp, ex2capp: bool, err_msg: str | None, test_app_cls):
    """Test the initialisation of ConsoleApp."""
    if err_msg is not None:
        with pytest.raises(ValueError, match=err_msg):
            test_app_cls(capp, ex2capp)
    else:
        app_instance = test_app_cls(capp, ex2capp)
        assert app_instance.app_name == "Test App"
        assert app_instance.calling_app == capp
        assert app_instance.exit_to_calling_app == ex2capp
